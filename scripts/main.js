...
(function() {
  var Game, Random, Vector2, difficulty, game, settings;
  ...
  Game = (function() {
    ...
    Game.prototype.loadSettings = function(InputSettings) {
      ...
      if (settings != null) {
        ...
        if (this.mines > (this.sizeX * this.sizeY)) {
          this.mines = this.sizeX * this.sizeY;
        }
        this.resizeBorder();
        return true;
      }
  ...
  $('a.toggle-rules').click(function() {
    return $('#rulesModal').simplePopup();
  });

  $('#load-new-settings').click(function() {
    var specialDif;
    if ($('#dif-level-beginner').is(":checked")) {
      ...
      specialDif = {
        sizeY: Number($('#special-height').val()) || 20,
        sizeX: Number($('#special-width').val()) || 40,
        mines: Number($('#special-mines').val()) || 145
      };
  ...
  $('[name=zoom]').change(function() {
    if (this.value === "100" || this.value === "150" || this.value === "200") {
      return game.loadSettings({
        zoom: Number(this.value)
      });
    }
  });

  $('#special-height, #special-width, #special-mines').focus(function() {
    return $('#dif-level-special').prop('checked', true);
  });
